...
def main():
    ...
    train_loader, test_loader, train_eval_loader = get_loaders(
        train_csv_path=config.DATASET+"/new_train.csv", test_csv_path=config.DATASET+"/new_val.csv"
        # train_csv_path=config.DATASET+"/train.csv", test_csv_path=config.DATASET+"/val.csv"
    )
    ...
    for epoch in range(config.NUM_EPOCHS):
        # train_fn(test_loader, model, optimizer,
        ...
        if epoch % 10 == 0 and epoch > 0:
            ...
            pred_boxes, true_boxes = get_evaluation_bboxes(
                test_loader, model, iou_threshold=config.NMS_IOU_THRESH,
